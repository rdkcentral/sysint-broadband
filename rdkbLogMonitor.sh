...
#DEBUG_LOG="/tmp/.rdkbLogMonitor_debug.txt"
...
generate_random_sleep()
{
    ...
    echo_t "Random sleep for $RANDOM_SLEEP seconds"
}
...
random_sleep()
{
    ...
    if [ "$CRON_MODE" = "1" ]; then
	    if [ ! -f "$RANDOM_DELAY_FILE" ]; then
            ...
            echo_t "rdkbLogMonitor: Initial random delay stored: $RANDOM_SLEEP seconds"
        else
            echo_t "rdkbLogMonitor: Random already generated, reusing existing value"
        fi
        ...
        if [ "$current_tick" = "0" ]; then
            ...
            if [ "$remaining" -le 300 ]; then
				echo_t "rdkbLogMonitor: Sleeping $remaining seconds NOW"
				rm -f "$TICK_FILE"
				delay_completed=1
				[ "$remaining" -gt 0 ] && sleep "$remaining"
            else
                echo_t "rdkbLogMonitor: Remaining sleep before completion: $remaining seconds"
                new_remaining=$((remaining - 300))
                if [ "$new_remaining" -lt 0 ]; then
                    new_remaining=0
                fi
                echo $new_remaining > "$RANDOM_DELAY_FILE"
                echo_t "rdkbLogMonitor: Updated remaining sleep to $new_remaining seconds, exiting"
            fi
        else
            echo_t "rdkbLogMonitor: Skipping this minute (tick=$current_tick/4)"
        fi
	    ...
	    generate_random_sleep
        echo_t "rdkbLogMonitor: Sleeping for $RANDOM_SLEEP seconds"
        sleep $RANDOM_SLEEP
    fi
}
	...
	if [ "$LOGBACKUP_ENABLE" == "true" ]; then
		...
		if [ -d "$LOG_BACK_UP_REBOOT" ]; then
			...
			then
				...
				if [ "$BOX_TYPE" == "HUB4" ] || [ "$BOX_TYPE" == "SR300" ] || [ "x$BOX_TYPE" == "xSR213" ] || [ "$BOX_TYPE" == "SE501" ] || [ "$BOX_TYPE" == "WNXL11BWL" ] || [ "$BOX_TYPE" == "SCER11BEL" ] || [ "$BOX_TYPE" == "SCXF11BFL" ]; 
				then
...
bootup_upload_on_reboot()
{
    ...
    if [ -e "$UPLOAD_ON_REBOOT" ]; then
		if [ "$CRON_MODE" = "1" ]; then 
			if [ -f "$RANDOM_DELAY_FILE" ]; then
				echo_t "random_sleep active during Step 2..."
				if [ -f "$TICK_FILE" ]; then
					random_sleep
					if [ $? -eq 0 ]; then
						echo_t "random_sleep incomplete - next cron"
						return 0
					fi
				fi
				echo_t "random_sleep completed"
				rm -f "$RANDOM_DELAY_FILE"
				$RDK_LOGGER_PATH/uploadRDKBLogs.sh $SERVER "HTTP" $URL "true" "" $TMP_LOG_UPLOAD_PATH "true"
			else
				echo_t "First time - executing prep + sleep"
				file_bootup_upload_on_reboot
			...
			echo_t "First time - executing prep + sleep - Service"
			file_bootup_upload_on_reboot
			if [ "$fileToUpload" != "" ]; then
				random_sleep
				$RDK_LOGGER_PATH/uploadRDKBLogs.sh $SERVER "HTTP" $URL "true" "" $TMP_LOG_UPLOAD_PATH "true"
	...
	if [ "$CRON_MODE" = "1" ]; then
		touch $RDKB_LOGMON_TMP_DIR/.api1_complete
	fi
}
...
tar_bootup_upload()
{
    echo_t "RDK_LOGGER: bootup_upload_2"
    echo_t "Check if any tar file available in /logbackup/ "
    curDir=`pwd`
	if [ "$CRON_MODE" = "1" ]; then
		if [ -f "$RANDOM_DELAY_FILE" ]; then
			echo_t "bootup_upload_2: RESUMING random_sleep"
			if [ -f "$TICK_FILE" ]; then
				random_sleep
				if [ $? -eq 0 ]; then
					echo_t "random_sleep incomplete - next cron"
			...
			if [ "$CRON_MODE" = "1" ]; then
				touch $RDKB_LOGMON_TMP_DIR/.api2_complete
			fi
		else
			echo_t "bootup_upload_2: FIRST TIME prep"
			tar_file_bootup_upload

			echo_t "Prep done: UploadFile='$UploadFile' preserve=$files_exist_in_preserve"
			echo_t "File to be uploaded from logbackup/ is $UploadFile "
			if [ "$UploadFile" != "" ]; then
				if [ "$UPLOADED_AFTER_REBOOT" == "true" ]; then
					random_sleep
		...
		echo_t "bootup_upload_2: FIRST TIME prep - SERVICE"
		tar_file_bootup_upload
	...
	if ( [ "$file_list" != "" ] && [ ! -f "$UPLOAD_ON_REBOOT" ] ) || ( [ "$RebootReason" == "factory-reset" ] ); then
		...
		if [ "$BOX_TYPE" == "HUB4" ] || [ "$BOX_TYPE" == "SR300" ] || [ "x$BOX_TYPE" == "xSR213" ] || [ "$BOX_TYPE" == "SE501" ] || [ "$BOX_TYPE" == "WNXL11BWL" ] || [ "$BOX_TYPE" == "SCER11BEL" ] || [ "$BOX_TYPE" == "SCXF11BFL" ]; then
			...
			do
				...
				TIME_SYNC_STATUS=`timedatectl status | grep "clock synchronized" | cut -d " " -f 4`
...
service_mode() {
    echo_t "rdkbLogMonitor.sh - Running in SERVICE mode"
...
if [ "$rdklogger_cron_enable" = "true" ]; then
    echo_t "rdkbLogMonitor.sh - Running in CRON mode"
    CRON_MODE=1
    ...
    if [ ! -f "$BOOT_PROCESSED_FLAG" ]; then
        echo_t "CRON: First boot - Sequential bootup processing"
        
        if [ ! -f "$RDKB_LOGMON_TMP_DIR/.tarlogs_done" ]; then
            echo_t "CRON: Step 1 - bootup_tarlogs + INITIAL monitoring"

			bootup_nvram_upload
            bootup_tarlogs
            touch "$RDKB_LOGMON_TMP_DIR/.tarlogs_done"
        fi

        if [ -f "$RDKB_LOGMON_TMP_DIR/.tarlogs_done" ] && [ ! -f "$RDKB_LOGMON_TMP_DIR/.api1_complete" ]; then
            echo_t "CRON: Step 2 - bootup_upload_1 + threshold check"
            
			if [ ! -f "$RDKB_LOGMON_TMP_DIR/.firstbootup_done" ]; then
				bootup_upload_on_reboot
				device_state
				regular_upload_state
				touch "$RDKB_LOGMON_TMP_DIR/.firstbootup_done"
			else
				bootup_upload_on_reboot
				getLogfileSize "$LOG_PATH"
				if [ "$totalSize" -ge "$MAXSIZE" ]; then
					echo_t "THRESHOLD HIT during Step 2! Regular upload..."
					device_state
					regular_upload_state
				fi
			fi
        ...
        if [ -f "$RDKB_LOGMON_TMP_DIR/.api1_complete" ] && [ ! -f "$RDKB_LOGMON_TMP_DIR/.api2_complete" ]; then
            echo_t "CRON: Step 3 - bootup_upload_2 + threshold check"
        ...
        if [ -f "$RDKB_LOGMON_TMP_DIR/.api2_complete" ]; then
            echo_t "CRON: Bootup processing complete"
            touch "$BOOT_PROCESSED_FLAG"
            rm -f "$RDKB_LOGMON_TMP_DIR/.tarlogs_done" \
                  "$RDKB_LOGMON_TMP_DIR/.api1_complete" \
                  "$RDKB_LOGMON_TMP_DIR/.api2_complete"
        fi
    fi

    echo_t "CRON: Regular processing (post-bootup)..."
    
    getLogfileSize "$LOG_PATH"
    echo_t "CRON: Log totalSize=$totalSize, MAXSIZE=$MAXSIZE"
    
    if [ "$totalSize" -ge "$MAXSIZE" ]; then
        echo_t "CRON: THRESHOLD HIT! Regular upload after bootup..."
        device_state
        regular_upload_state
        echo_t "CRON: Regular upload completed"
    else
        echo_t "CRON: Logs below threshold ($totalSize < $MAXSIZE)"
    fi
    
    exit 0
else
    CRON_MODE=0
    echo_t "Running in SERVICE mode"
    service_mode  # Original background behavior
fi
