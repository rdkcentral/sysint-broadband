...
install_cron_entry() {
    CRON_LINE="* * * * * /rdklogger/rdkbLogMonitor_cron.sh"
    
    if crontab -l 2>/dev/null | grep -q "rdkbLogMonitor_cron.sh"; then
        echo_t "rdkbLogMonitor_cron.sh - Cron entry already present"
        return 0
    fi
    ...
    if [ $rc -eq 0 ]; then
        echo_t "rdkbLogMonitor_cron.sh - Cron installed cleanly: $CRON_LINE"
    else
        echo_t "rdkbLogMonitor_cron.sh - Cron install failed (rc=$rc)"
    fi
}
