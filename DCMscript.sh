...
if [ "x$T2_ENABLE" == "xtrue" ]; then
    ...
    if [ -z "$t2Pid" ]; then
        echo "${T2_0_APP} is present and running with pid : ${t2pid}, XCONF config fetch and parse will be handled by T2 implementation" >> $DCM_LOG_FILE
        t2Log "Clearing markers from $TELEMETRY_PATH"
        rm -rf $TELEMETRY_PATH
	rm -rf /tmp/.t2ConfigReady
        ...
        if [ -f $DEFAULT_REPORT_FILE ]; then
            while [ ! -f /tmp/.t2ConfigReady ] && [ ! -f /tmp/t2DcmComplete ]
            do
